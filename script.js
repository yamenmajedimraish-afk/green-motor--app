function calculate() {
  ...
  const EF = {
    ...
    planes: 9000, lighting: 0.42, heating: 0.20,
    cooling: 0.42, computing: 0.42
  };

  // Emission calculations (tons CO2e)
  ...
  const optimizedTotal = carsEm + trucksEm + busesEm + forkliftsEm + planesEm +
    lightingEm + heatingEm + coolingEm + computingEm + subcontractorEm;

  const baselineTotal = optimizedTotal * 1.5;

  const labels = ["Cars", "Trucks", "Buses", "Forklifts", "Cargo Planes", "Lighting", "Heating", "Cooling", "Computing", "Subcontractors"];
  const data = [carsEm, trucksEm, busesEm, forkliftsEm, planesEm, lightingEm, heatingEm, coolingEm, computingEm, subcontractorEm];

  renderCharts(labels, data, baselineTotal, optimizedTotal);
}
...
function renderCharts(labels, data, baseline, optimized) {
  ...
  window.pieChart = new Chart(ctxPie, {
    type: "pie",
    data: { labels, datasets: [{ data, backgroundColor: ["#4caf50", "#ff9800", "#2196f3", "#9c27b0", "#f44336", "#00bcd4", "#cddc39", "#ffeb3b", "#795548", "#9e9e9e"] }] },
    options: { plugins: { legend: { labels: { color: "#fff" } } } }
  });

  window.barChart = new Chart(ctxBar, {
    type: "bar",
    data: { labels: ["Baseline", "Optimized"], datasets: [{ label: "Total Emissions (tons CO₂e)", data: [baseline, optimized], backgroundColor: ["#2196f3", "#4caf50"] }] },
    options: {
